...
public class Ex07 {
	...
	public static void main(String[] args) {
		...
		Arrays.sort(arr2, null);	// arr2를 이런 방식으로 정렬해라 -> 방식을 제시하지 않았음(null) -> 그렇지만 정렬이 됨 -> 기본 오름차순
	...
	static void SelectionSortOrderBySumDesc(Student[] arr) {
		for(int i = 0; i < arr.length-1; i++) {
			for(int j = i + 1; j < arr.length; j++) {
				Student a = arr[i];
				Student b = arr[j];
				if(a.sum - b.sum < 0) {		// 내림차순 (ex) 200-250 < 0 성립하니까 바꿈/ 250-200 < 0 성립하지 않으니까 안 바꿈)
					Student tmp = arr[i];
					arr[i] = arr[j];
					arr[j] = tmp;
				}
