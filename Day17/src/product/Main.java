	...
	public static void main(String[] args) {
		...
		while(true) {
			...
			case 2:		// 추가 (입력받은 데이터를 객체화해서 Handler에게 전달)
				...
				menu = Integer.parseInt(sc.nextLine());
				if(menu == 1) 		tmp = makeFood(sc);		// 앞에 객체가 없기 때문에 static으로 써야하고, main이 static이기 때문에 사용하려면 static으로 생성해야 함
			...
			case 3:		// 검색 (입력받은 검색어를 전달하여 조건(포함)에 맞는 배열을 받아오기)
				System.out.println("검색어 (상품이름) 입력 : ");
				name = sc.nextLine();
			...
			case 5:		// 정렬 (상품 이름 순으로 정렬하기)
				...
				// a : 97
				// 0 : 48
				// 1, 2번을 사용할 때는 입력 받을 필요 없음
				System.out.println("정렬 기준 선택 (1 ~ 4) : ");
				menu = Integer.parseInt(sc.nextLine());
				
				arr = handler.sort(menu - 1);		// 1,2번을 사용할 때는 menu - 1을 안써도 됨
				for(int i = 0; i < arr.length; i++) {
					if(arr[i] != null) {
