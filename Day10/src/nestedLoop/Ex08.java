...
public class Ex08 {
	...
	public static void main(String[] args) {
		...
		int[][] arr = new int[5][5];		//int[][] arr = new int[4][6];	4줄 6칸
		show(arr);
		...
		for(int i = 0; i < arr.length; i++) {
			for(int j = 0; j < arr[i].length; j++) {
				if(i % 2 == 0) {			// i는 가로 j가 세로니까 짝수 줄은 정상적으로 출력
					arr[i][j] = num++;					
				}
				else { 							// j : 0, 1, 2, 3, 4
		...
		arr[0][0] = num++;		// i값이 짝수이면
		arr[1][0] = num++;		// y값은 0부터 1씩 증가하여 4까지
		arr[2][0] = num++;
		arr[3][0] = num++;
		arr[4][0] = num++;
		
		arr[4][1] = num++;		// i값이 홀수이면
		...
		arr[2][2] = num++;		// 		i가 짝수이면 그대로 0부터 4까지 증가하는대로 사용
		arr[3][2] = num++;		// 		j가 홀수이면 4부터 0까지 감소하는 형식이 되어야 한다
		arr[4][2] = num++;
