...
class Car { 
	Human[] seat = new Human[4];		// 사람이 앉을 수 있는 배열 4칸
...
public class Ex07 {
	public static void main(String[] args) {
		...
		car.entrance(st2);		// 매개변수의 자료형이 슈퍼클래스이므로 -> void entrance(Human ob)
		car.entrance(ac1);		// 함수를 호출하면서 업캐스팅이 발생한다
		car.entrance(ac2);	
		
		Human t1 = ac1;			// 서브클래스의 객체를 슈퍼클래스 타입으로 참조한다(업캐스팅)
		Human t2 = st1;			// 각각 다른 타입의 객체를 묶어서 처리하고 싶을때 업캐스팅 
		...
		// 4. 다형성 : 하나의 객체는 슈퍼클래스로 참조되거나, 서브클래스로 참조될 수 있다(상속의 오버라이딩과 연결되서 많이 씀)
