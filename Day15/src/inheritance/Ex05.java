...
// 상속은 물리적인 관계 포함은 xx
// 예를 들어 타이어는 자동차 부품이다 포함 관계이지만 타이어가 자동차인건 아니다(상속 불가)
// 예를 들어 학생은 사람에 포함되고 학생은 사람이다가 가능하기 때문에 상속 가능
...
//	Cannot reduce the visibility of the inherited method from Calc
	// 1) 상속받은 메서드의 접근 제한 범위를 더 줄일 수 없다(넓히기는 가능) - 안적으면 package이기 때문에 protected보다 범위가 좁다(그래서 public을 써줌)
	// 2) 상속받은 메서드의 반환형과 일치해야 한다
	// 3) 상속받은 메서드의 이름과 매개변수 유형 및 개수가 정확하게 일치해야 한다
	// 4) 슈퍼클래스로 참조해도 메서드를 호출하면 서브 클래스에서 정의한대로 실행된다(다형성)
...
public class Ex05 {
	public static void main(String[] args) {
		...
		// 4번 특징
		// 서브클래스의 객체는 슈퍼클래스 타입이다(다형성)
		// 학생은 사람이다
		// 고양이는 동물이다
		// 강아지는 동물이다
		// ob2는 Calc이다
		
		Calc test = ob2;			// test의 자료형은 Calc(슈퍼클래스)이지만
